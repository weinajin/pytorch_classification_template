...
#         img_seg = trF.resize(image_seg, self.size, Image.NEAREST)
        return {'image': img, 'image_seg': img, 'label': label}
...
#         img_seg = None # trF.center_crop(image_seg, new_size)
        return {'image': img, 'image_seg': img, 'label': label}
...
#         img_seg = None #  trF.center_crop(image_seg, self.size)
        return {'image': img, 'image_seg': img, 'label': label}
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': img, 'image_seg': image, 'label': label}
